...
void RasterizerUtils::_bind_methods() {
    ...
    ClassDB::bind_static_method("RasterizerUtils", 
        D_METHOD("rasterize_tile_data_safe_fast_butZfight", "safe_mvt_data", "config"), 
        &RasterizerUtils::rasterize_tile_data_safe_fast_butZfight);
    ClassDB::bind_static_method("RasterizerUtils", 
        D_METHOD("rasterize_tile_data_safe_slow", "safe_mvt_data", "config"), 
        &RasterizerUtils::rasterize_tile_data_safe_slow);
    ClassDB::bind_static_method("RasterizerUtils", 
...
Variant RasterizerUtils::rasterize_tile_data_safe(const Dictionary &safe_mvt_data, const Dictionary &config) {
    ...
    // ... (Konfigurations-Variablen bleiben gleich) ...
    ...
    if (sub_chunks_to_render.is_empty()) return String();
    const double tile_extent = safe_mvt_data.get("tile_extent", 4096.0);
    const Array layers = safe_mvt_data.get("layers", Array());
    if (layers.is_empty()) return String();
    const int64_t STATE_CANCELLED = 5;
    Object *tile_state_obj = config.get("tile_state", Variant());

    // Initialisiere Sub-Chunks (logic_map)
    ...
    // Gruppiere Features nach Typ (unverändert)
    Dictionary all_features_by_type;
    // ... (Code zum Füllen von all_features_by_type bleibt identisch) ...
    ...
    // --- PHASE 1: RASTERISIERUNG ZUR ERZEUGUNG DER KORREKTEN LOGIC_MAP ---
    ...
    for (int i = 0; i < render_order.size(); ++i) {
        ...
        for (int j = 0; j < features_of_type.size(); ++j) {
            // ... (Abbruch-Prüfung, Geometrie-Parsing, Edge-Tabelle bleiben identisch) ...
            // ... (Der ganze Code bis zur Rasterisierungs-Schleife) ...
            if (tile_state_obj != nullptr && j % 20 == 0) { if ((int64_t)tile_state_obj->get("current_state") == STATE_CANCELLED) return String(); }
            Dictionary feature_data = features_of_type[j];
            Array geometry_commands = feature_data.get("geometry", Array());
            Array all_rings_pixel_space; PackedVector2Array current_ring; Vector2 cursor(0, 0);
            for (int k = 0; k < geometry_commands.size(); ++k) {
                Array cmd_seq = geometry_commands[k]; if (cmd_seq.is_empty()) continue; int64_t cmd_id = cmd_seq[0];
                if (cmd_id == 1) { if (cmd_seq.size() >= 3) { cursor += Vector2(cmd_seq[1], cmd_seq[2]); current_ring.push_back(cursor * scale_vec_to_pixel); } }
                else if (cmd_id == 2) { for (int l = 1; l < cmd_seq.size(); l += 2) { if (l + 1 < cmd_seq.size()) { cursor += Vector2(cmd_seq[l], cmd_seq[l + 1]); current_ring.push_back(cursor * scale_vec_to_pixel); } } }
                else if (cmd_id == 7) { if (!current_ring.is_empty()) { all_rings_pixel_space.push_back(current_ring); current_ring = PackedVector2Array(); } }
            }
            if (all_rings_pixel_space.is_empty()) continue;
            Rect2 feature_bounds_pixels; std::map<int, std::vector<Edge>> edge_table;
            for (int k = 0; k < all_rings_pixel_space.size(); ++k) {
                PackedVector2Array ring = all_rings_pixel_space[k];
                for (int v = 0; v < ring.size(); ++v) {
                    Vector2 p1 = ring[v]; Vector2 p2 = ring[(v + 1) % ring.size()]; if (p1.y > p2.y) std::swap(p1, p2); double delta_y = p2.y - p1.y; if (abs(delta_y) < 1e-9) continue;
                    Edge edge; edge.y_max = p2.y; edge.x_current = p1.x; edge.inv_slope = (p2.x - p1.x) / delta_y; edge_table[static_cast<int>(ceil(p1.y))].push_back(edge);
                    if (k == 0 && v == 0) { feature_bounds_pixels = Rect2(p1, Vector2()); } feature_bounds_pixels.expand_to(p1); feature_bounds_pixels.expand_to(p2);
                }
            }
            Vector2i start_chunk = (feature_bounds_pixels.get_position() / sub_chunk_resolution).floor(); Vector2i end_chunk = ((feature_bounds_pixels.get_position() + feature_bounds_pixels.get_size()) / sub_chunk_resolution).floor();
            std::map<Vector2i, uint8_t*> logic_map_pointer_cache;
            for (auto& pair : prepared_sub_chunks_map) {
                const Vector2i& coord = pair.first; if (coord.x >= start_chunk.x && coord.x <= end_chunk.x && coord.y >= start_chunk.y && coord.y <= end_chunk.y) { logic_map_pointer_cache[coord] = pair.second.logic_map_bytes.ptrw(); }
            }
            std::vector<Edge> active_edge_list; int y_start = static_cast<int>(ceil(feature_bounds_pixels.position.y)); int y_end = static_cast<int>(ceil(feature_bounds_pixels.get_end().y));

            for (int y = y_start; y < y_end; ++y) {
                if (edge_table.count(y)) { active_edge_list.insert(active_edge_list.end(), edge_table[y].begin(), edge_table[y].end()); }
                active_edge_list.erase(std::remove_if(active_edge_list.begin(), active_edge_list.end(), [y](const Edge& edge) { return y >= edge.y_max; }), active_edge_list.end());
                ...
                for (size_t e = 0; e + 1 < active_edge_list.size(); e += 2) {
                    ...
                    for (int x = x_start; x < x_end; ++x) {
                        if (x < 0 || x >= total_pixel_dim || y < 0 || y >= total_pixel_dim) continue;
                        Vector2i sub_chunk_coord(x / sub_chunk_resolution, y / sub_chunk_resolution);
                        auto it = logic_map_pointer_cache.find(sub_chunk_coord);
                        if (it != logic_map_pointer_cache.end()) {
                            int pixel_x = x % sub_chunk_resolution;
                            int pixel_y = y % sub_chunk_resolution;
                            // NUR die logic_map schreiben. Keine Datensammlung hier!
                            it->second[pixel_y * sub_chunk_resolution + pixel_x] = type_id;
                        }
                    }
                }
                for (auto& edge : active_edge_list) { edge.x_current += edge.inv_slope; }
            }
        }
    }
    
    // --- PHASE 2: SCHNELLE DATENGENERIERUNG AUS DER FERTIGEN LOGIC_MAP ---
    Dictionary final_sub_chunks;
    for (auto const& [coord, chunk_data_struct] : prepared_sub_chunks_map) {
        // Native C++-Struktur zum schnellen Sammeln
        std::unordered_map<int64_t, std::vector<Vector3>> positions_by_type_native;
        const uint8_t* logic_map_ptr = chunk_data_struct.logic_map_bytes.ptr();

        // Iteriere über die finale logic_map
        for (int64_t y = 0; y < sub_chunk_resolution; ++y) {
            for (int64_t x = 0; x < sub_chunk_resolution; ++x) {
                ...
                if (type_id > 0) {
                    // Sammle Positionen im schnellen C++ Vektor
                    positions_by_type_native[type_id].emplace_back(
                        x * scale_factor, 0, y * scale_factor
                    );
                }
            }
        }

        // --- Finale Konvertierung (einmalig pro Chunk) ---
        Dictionary vegetation_buffers;
        for (const auto& [type_id, positions_native] : positions_by_type_native) {
            int64_t instance_count = positions_native.size();
            ...
            for (int k = 0; k < instance_count; ++k) {
                const Vector3 &pos = positions_native[k];
                Basis basis = Basis().scaled(Vector3(scale_factor, 1.0, scale_factor));
                int64_t buffer_start_index = k * STRIDE;
                // ... (Buffer füllen, identisch wie vorher) ...
                data_buffer.set(buffer_start_index + 0, basis.get_column(0).x); data_buffer.set(buffer_start_index + 1, basis.get_column(1).x); data_buffer.set(buffer_start_index + 2, basis.get_column(2).x); data_buffer.set(buffer_start_index + 3, pos.x);
                data_buffer.set(buffer_start_index + 4, basis.get_column(0).y); data_buffer.set(buffer_start_index + 5, basis.get_column(1).y); data_buffer.set(buffer_start_index + 6, basis.get_column(2).y); data_buffer.set(buffer_start_index + 7, pos.y);
                data_buffer.set(buffer_start_index + 8, basis.get_column(0).z); data_buffer.set(buffer_start_index + 9, basis.get_column(1).z); data_buffer.set(buffer_start_index + 10, basis.get_column(2).z); data_buffer.set(buffer_start_index + 11, pos.z);
                data_buffer.set(buffer_start_index + 12, rand() % 16); data_buffer.set(buffer_start_index + 13, 0.0); data_buffer.set(buffer_start_index + 14, 0.0); data_buffer.set(buffer_start_index + 15, 1.0);
            }
...
Variant RasterizerUtils::rasterize_tile_data_safe_fast_butZfight(const Dictionary &safe_mvt_data, const Dictionary &config) {
    ...
    // --- NEU: Datenstruktur zum Sammeln der Vegetationspositionen ---
    // Key: Sub-Chunk-Koordinate, Value: Dictionary(Key: type_id, Value: PackedVector3Array)
    std::map<Vector2i, std::unordered_map<int64_t, std::vector<Vector3>>> vegetation_positions_map;
    // Initialisiere die Map, um Checks in der Schleife zu vermeiden
    for (int i = 0; i < sub_chunks_to_render.size(); ++i) {
        vegetation_positions_map[sub_chunks_to_render[i]] = {};
    }
    ...
    for (int i = 0; i < render_order.size(); ++i) {
        ...
        for (int j = 0; j < features_of_type.size(); ++j) {
            ...
            // Geometrie-Parsing und Edge-Tabelle (unverändert)
            Dictionary feature_data = features_of_type[j];
            Array geometry_commands = feature_data.get("geometry", Array());
            ...
            for (int k = 0; k < all_rings_pixel_space.size(); ++k) {
                PackedVector2Array ring = all_rings_pixel_space[k];
                for (int v = 0; v < ring.size(); ++v) {
                    Vector2 p1 = ring[v]; Vector2 p2 = ring[(v + 1) % ring.size()];
                    if (p1.y > p2.y) std::swap(p1, p2);
                    double delta_y = p2.y - p1.y;
                    if (abs(delta_y) < 1e-9) continue;
                    Edge edge; edge.y_max = p2.y; edge.x_current = p1.x; edge.inv_slope = (p2.x - p1.x) / delta_y;
                    edge_table[static_cast<int>(ceil(p1.y))].push_back(edge);
                    if (k == 0 && v == 0) { feature_bounds_pixels = Rect2(p1, Vector2()); }
                    feature_bounds_pixels.expand_to(p1); feature_bounds_pixels.expand_to(p2);
                }
            }
            ...
            for (auto& pair : prepared_sub_chunks_map) {
                ...
                if (coord.x >= start_chunk.x && coord.x <= end_chunk.x && coord.y >= start_chunk.y && coord.y <= end_chunk.y) {
                ...
                for (size_t e = 0; e + 1 < active_edge_list.size(); e += 2) {
                    ...
                    int x_end = static_cast<int>(ceil(active_edge_list[e + 1].x_current));
                     ...
                     for (int x = x_start; x < x_end; ++x) {
                        ...
                        int chunk_x = x / sub_chunk_resolution;
                        int chunk_y = y / sub_chunk_resolution;
                        ...
                        if (it != logic_map_pointer_cache.end()) {
                            ...
                            it->second[index] = type_id;

                            // --- KORREKTUR: Schnelle Sammlung in native C++ Vektoren ---
                            if (type_id > 0) {
                                // Holen der Referenz ist schnell, keine Kopie!
                                auto& positions_for_type = vegetation_positions_map.at(sub_chunk_coord);
                                // emplace_back ist effizienter als push_back
                                positions_for_type[type_id].emplace_back(
                                    pixel_x * scale_factor, 0, pixel_y * scale_factor
                                );
                            }
                        }
                    }
    ...
    // --- ANGEPASSTE Finale Datenaufbereitung ---
    Dictionary final_sub_chunks;
    for (auto const& [coord, chunk_data_struct] : prepared_sub_chunks_map) {
        
        
        Dictionary vegetation_buffers;
        const auto& positions_by_type_native = vegetation_positions_map.at(coord);

        for (const auto& [type_id, positions_native] : positions_by_type_native) {
            int64_t instance_count = positions_native.size();
            if (instance_count == 0) continue;
            
            // Der Rest der Buffer-Erstellung ist identisch
            const int64_t STRIDE = 16;
            PackedFloat32Array data_buffer;
            data_buffer.resize(instance_count * STRIDE);
...
Variant RasterizerUtils::rasterize_tile_data_safe_slow(const Dictionary &safe_mvt_data, const Dictionary &config) {
    // Früher Ausstieg bei leeren Daten
    if (safe_mvt_data.is_empty()) {
        UtilityFunctions::push_warning("RasterizerUtils: Sichere MVT-Daten sind leer.");
        return String(); // Leerer String statt PackedByteArray
