...
func _process_current_creation_task():
	for frame in 1:
		...
		if not is_instance_valid(tile_node):
				# Der Parent-Node wurde aus irgendeinem Grund gelöscht.
				# Wir können diesen Job nicht fortsetzen.
		...
		var tile_extent = 8192.0
		var p_scale = TILE_PIXEL_SIZE / tile_extent
		feature_node.scale = Vector3(p_scale, p_scale, 1.0)
		feature_node.use_collision = true
		#feature_node.bake_static_mesh()
...
func _multimesh_test(prepared_data)->void:
	var multimesh_inst: MultiMeshInstance3D = MultiMeshInstance3D.new()
	var multimesh: MultiMesh = MultiMesh.new()
	multimesh.transform_format = MultiMesh.TRANSFORM_3D
	var tile_mesh: Mesh = PlaneMesh.new()
	tile_mesh.size = Vector2(256,256)
	tile_mesh.orientation = PlaneMesh.FACE_Y
	multimesh.mesh = tile_mesh
	multimesh_inst.multimesh = multimesh
	multimesh_inst.multimesh.use_custom_data = true
	multimesh_inst.multimesh.instance_count = TILE_PIXEL_SIZE**2

	var polygons: Array = prepared_data.get("polygons")
	var holes: Array = prepared_data.get("holes")
	for x in TILE_PIXEL_SIZE:
		for y in TILE_PIXEL_SIZE:
			var point: Vector2i = Vector2i(x,y)
			var set_tile: bool = false
			for polygon in polygons:
				set_tile = Geometry2D.is_point_in_polygon(Vector2i(x,y),polygon)
			for hole in holes:
				set_tile = !Geometry2D.is_point_in_polygon(Vector2i(x,y),hole)
			if set_tile:
				multimesh_inst.multimesh.set_instance_transform(x,Transform3D(Basis.IDENTITY, Vector3(x,0.0,y)))
	self.add_child(multimesh_inst)
	#for polygon in polygons:
